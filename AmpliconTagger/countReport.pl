...
--barcodesDist <string>    : Barcodes distribution
--Featuretable <string>    : Classified Features/clusters. (Feature table).
...
## OPTIONS
my ($help, @analysisType, @barcodesDist, @Featuretable, @obsTable, @name, @file);
my $verbose = 0;
...
foreach my $analysisType (@analysisType){
  ...
  my $Featuretable = shift(@Featuretable);
  ...
  open(IN, $Featuretable) or die "Can't open file ".$Featuretable."\n";
  $line_number = 0;
  $sum=0;
  while(<IN>){
    chomp($_);
