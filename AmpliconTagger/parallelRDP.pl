...
## OPTIONS
my ($help, $infile, $outfile, $rdp_training_set, $minWords, $num_threads, $fasta, $ram);
...
my $tmpdir = File::Temp->newdir(
    "tmpDirAmpliconTaggerRDPXXXXXXX",
    DIR => $TMPDIR."/",
...
if($fasta){
  ...
  $cmd .= " > ".$outfile;
  system($cmd);
  $? != 0 ? die "command failed: $!\n" : print STDERR "Concatenating RDP sheets done\n";  
