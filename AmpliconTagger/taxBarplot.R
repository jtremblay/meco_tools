...
generatePlots <- function(infile, outdir, prefix) {
    ...
    tData = data.frame(fread(infile, header=TRUE, sep="\t"), check.names=FALSE)
	...
	outfile2 <- paste0(outdir, prefix, ".png")
	...
	png( file=outfile2, height=8, width=16, units="in", res=300)
	print(p)
	dev.off()
...
usage=function(errM) {
        cat("\nUsage : Rscript taxBarplot.R [option] <Value>\n")
        cat("       -i        : infile\n")
        cat("       -o        : outdir\n")
        cat("       -p        : prefix\n")
}
