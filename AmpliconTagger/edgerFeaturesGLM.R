...
performEdgerGLMFeatures <- function(infile, outdir, mappingFile, pvalue, fdr, logfc, blockColumnLabels, treatmentColumnLabels, cutoff=250) {
   library(edgeR)
   library(tools)
   library(data.table)
   library(statmod)
   ...
   for(curr_treatment_column in treatment_columns){
      ...
      print("y before:") 
      y <- DGEList(counts=tData2, remove.zeros=TRUE, group=groups)
      print("y after:") 
      print(y)
      ...
      if(identical(block_columns, character(0)) | block_columns == "no" ){ # means no blocks.
         ...
         for(i in 1:(length(unique_treatments)-1)){
            ...
            for(j in (i+1):length(unique_treatments)){
               ...
               write.table(DEGs2, file=paste0(curr_outdir, "/", curr_treatment,"_vs_", curr_control, "_edger.tsv"), quote=FALSE, sep="\t", row.names=FALSE, col.names=TRUE)
               ...
               # Assign taxonomy to each DOA Features.
               DEGs2 = merge(DEGs1, tDataTaxonomy, by.x="symbol", by.y="row.names")            
               write.table(DEGs2, file=paste0(curr_outdir, "/", curr_control,"_vs_", curr_treatment, "_edger.tsv"), quote=FALSE, sep="\t", row.names=FALSE, col.names=TRUE)            
            }
         }
