...
my $usage = <<'ENDHERE';
NAME:
clusteringShortReadsDnaclust.pl
...
--chimera_detection <string> : usearch (for v6.0.294) or vsearch (v2.7.1)
...
previously clustering3.pl

ENDHERE

my ($help, $infile_fastq, $infile_fasta, $ref_db, $start_at, $verbose, $outdir, $barcodes, $num_threads, $lowAbunCutOff, $first_round, $second_round, $chimera_detection);
GetOptions(
  ...
  'chimera_detection=s' => \$chimera_detection,
  'verbose'             => \$verbose,
  'start_at=i'          => \$start_at,
  'help'                => \$help
);
...
my $vsearch;
my $usearch;
$chimera_detection = "" unless($chimera_detection);
if($chimera_detection eq "usearch"){
    $usearch= which('usearch'); chomp $usearch;
    print "path of usearch:\t".$usearch."\n" if($verbose);
    die "Can't find usearch on path\n" if(!defined($usearch));
}elsif($chimera_detection eq "vsearch"){
    $vsearch= which('vsearch'); chomp $vsearch;
    print "path of vsearch:\t".$vsearch."\n" if($verbose);
    die "Can't find vsearch on path\n" if(!defined($vsearch));
}else{
    print STDERR "--chimera_detection option: Only 'usearch' or 'vsearch' are supported at this time... Will use vsearch\n";
    $vsearch = which('vsearch'); chomp $vsearch;
    print "path of vsearch:\t".$vsearch."\n" if($verbose);
    die "Can't find vsearch on path\n" if(!defined($vsearch));
    $chimera_detection = "vsearch";
}
print STDERR "path of vsearch: ".$vsearch."\n" if($verbose);
print STDERR "path of usearch: ".$usearch."\n" if($verbose);
print STDERR "chimera_detection: ".$chimera_detection."\n" if($verbose);
...
#Chimera detection replaced with vsearch.
...
if($chimera_detection eq "usearch"){
    ...
    die "--chimera_detection has to be set to either 'usearch' or 'vsearch' \n";
}
...
print STDERR "Converting dnaclust output to fasta\n" if($start_at <= 9);
...
## Generate Obs table
writeClusters($outdir."/derep1_099_derep2_nonChimDeNovoRef_sorted_097_sorted.fasta", $outdir."/obs.tsv", $outdir."/obs.fasta");
