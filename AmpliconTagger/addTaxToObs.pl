...
and format the output as a FEATURE table compatible with the Qiime suite tools.
...
OUTPUT:
--outfile <tab_file>        :  FEATURE table compatible with the Qiime tools.
--outfile_failed <tab_file> :  FEATURE table compatible with the Qiime tools. 
...
#print OUT "#Full FEATURE Counts\n";
#print OUT_FAILED "#Full FEATURE Counts\n";
...
while(<RDP>){
  chomp($_);
  $_ =~ s/undef_undef_undef_undef_undef/undef/g;
  $_ =~ s/undef_undef_undef_undef/undef/g;
  $_ =~ s/undef_undef_undef/undef/g;
  $_ =~ s/undef_undef/undef/g;
  my @row = split(/\t/, $_);
  #print $_."\n" if($verbose);
  foreach my $el (@row){
    #print $el."\n" if($verbose);
  ...
  if(@row == 29){
    ...
    my $s = $row[26];
    $s =~ s/_/ /g;
    $hash{$id}           = "k__".$k.";p__".$row[11].";c__".$row[14].";o__".$row[17].";f__".$row[20].";g__".$row[23].";s__".$s;
    $hash_prob_best{$id} = $row[10]."\t".$row[13]."\t".$row[16]."\t".$row[19]."\t".$row[22]."\t".$row[25]."\t".$row[28];
