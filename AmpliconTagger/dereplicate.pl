...
## OPTIONS
my ($help, $fasta, $fastq, $outfile, $sort, $minsize, $num_threads, $minsize_per_sample);
my $verbose = 0;
...
my $tmpdir = File::Temp->newdir(
    "tmpDirAmpliconTaggerDereplicateXXXXXXX",
    DIR => $TMPDIR."/",
    CLEANUP => 0
);

$num_threads = 1 unless($num_threads);
$minsize_per_sample = 0; # Not implemented.
...
if($fastq){
  ...
  # Here, will simply split fastq in smaller fasta chunks.  
  ...
  ## Smaller fasta chunks will be feeded in constructHashFormFasta().
  ## Loop through fasta file parts and populate hash.
  constructHashFromFasta(@out_fhs);

}else{ ## Single threaded only and fasta.
