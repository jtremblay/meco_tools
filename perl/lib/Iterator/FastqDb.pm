...
sub header
{
    ...
    #print STDERR "Attempting to parse header\n";
	# illumina
    #                               @7001343F:157:CCW1LANXX:6:2212:1388:2202 1:N:0:CCTAAGAC+CCTAGAGT
    # Miseq dualindexing GQ         @M03555:234:000000000-BC247:1:2106:16499:2305 1:N:0:GGACTCCT+ACTGCATA
    if($hdr =~ /^@(\S+:\d+:\S+:\d+:\d+:\d+:\d+) ([12]):\S:\S+:([aAtTcCgGnN]+\+[aAtTcCgGnN]+)/){
		$this->{base}    = $1;
		$this->{barcode} = uc($3);
		$this->{pair}    = $2;
    #                @7001343F:157:CCW1LANXX:      6:2212:5611: 2243#CCTAAGAC+CCTAGAGT/1
    #                @M02509:  281:000000000-CCHYD:1:1101:15649:1331#ATTACTCG+GTACTGAC/1
    }elsif($hdr =~ /^@(\S+:\d+:\S+:\d+:\d+:\d+:\d+)#([aAtTcCgGnN]+\+[aAtTcCgGnN]+)\/([12])/){	
       #print STDERR "1\n";
		# barcoded, paired
		$this->{base}    = $1;
		$this->{barcode} = uc($2);
		$this->{pair}    = $3;
        #print STDERR "pattern 2 found\n";
        #@M02360:54:000000000-BNCFN:1:1101:13750:2266 1:N:0:0#TTACTATA
    }elsif($hdr =~ /^@(\S+:\d+:\S+:\d+:\d+:\d+:\d+) ([12]):\S:\S+:#([aAtTcCgGnN]+)/){
		$this->{base}    = $1;
		$this->{barcode} = uc($3);
		$this->{pair}    = $2;
        #print STDERR "FOUND NEW PATTERN!\n"

	}elsif($hdr =~ /^@(\S+:\d+:\d+:\d+:\d+)#([aAtTcCgGnN]+)\/([12])/){	
       #print STDERR "1\n";
		# barcoded, paired
		$this->{base}    = $1;
		$this->{barcode} = uc($2);
       ...
       #print STDERR  "4\n";
		...
		$this->{base}    = $1;
		$this->{barcode} = uc($2);
	}elsif ($hdr =~ /^@(\S+:\d+:\d+\S+:\d+:\d+:\d+:\d+) ([12]):\S:\S+:([aAtTcCgGnN]+)/){
       #print STDERR  "5\n";
		$this->{base}    = $1;
		$this->{pair}    = $2;	
		$this->{barcode} = uc($3);
      ...
      #print STDERR "7a";
       ...
       #print STDERR  "13\n";
        ...
        $this->{base} = $1;
        $this->{barcode} = uc($2);
	}elsif ($hdr =~ /^@(\S+R\d+)\/([12])/){ # For CAMI2 challenge
       #print STDERR  "14\n";
...
sub _check_qual
{
	...
	foreach my $seq (@{$this->{buffer}})
	{
		...
		foreach my $q (@qual)
		{
	     ...
	     if($ascii < 66) {  # extended pacbio Phred33+ I guess for ccs reads.
             ++$appears_sanger;
         }elsif ($ascii > 66 && $ascii < 73) {
             ++$appears_sanger;
         }elsif($ascii > 74) { 
             ++$appears_illumina;
         }
	...
	# SAVE RESULT
    #print STDERR "SANGER:".$appears_sanger."\n";
    #print STDERR "ILLUMINA:".$appears_illumina."\n";
	$this->{qc_params}->{qual_to_sanger} = $appears_sanger > $appears_illumina ? 0 : 1;
    #print STDERR "appears_sanger: ".$appears_sanger."\n";
    #print STDERR "appears_illumina: ".$appears_illumina."\n";
    if($appears_sanger > $appears_illumina){
        #print STDERR "SANGER WON!\n";
    }else{
        #print STDERR "ILLUMINA WON!\n";
    }
}
