...
# TODO make outfile_[12] optional
...
my $phred = iTagger::Utils->new($infile);
...
if( (-e $outfile_2.".temp") and (-s $outfile_2.".temp")){
   my $db = iTagger::FastqDb->new($outfile_2.".temp", {trimN=>0, trim3=>0}) or die("Unable to open Fastq file, ".$outfile_2.".temp\n");
...
sub worker {
   ...
   while( tell( $FASTQ ) < $end ) {
      ...
      ## Validate header
      my $validate = new iTagger::ValidateFastq($lines[0], $lines[3], $phred);
      $base = "@".$validate->base;   
