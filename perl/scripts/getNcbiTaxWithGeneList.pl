...
INPUT:
--gene_list <string>        : blast output table # TO DO
...
NOTES:
#~/scripts/getNcbiTaxWithGeneList.pl --accession_list ./myaccession_list.table --accession_to_tax ./gi_taxid_nucl.dmp --names ../ncbi_tax2/taxa.sqlite.dmp > ./stdout.txt 2> stderr.txt
...
## OPTIONS
my ($help, $accession_list, $accession_to_tax, $names, $cutoff, $length, $include_blast_results);
my $verbose = 0;
...
while(<IN>){
   ...
   $hash_evalues{$_} = 1e-99;
   $hash_lengths{$_} = 500;
   $hash_bitscores{$_} = 500;
