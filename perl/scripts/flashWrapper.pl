...
## OPTIONS
my ($help, $infile_1, $infile_2, $num_threads, $outdir, $start_at, $n, $sampleSheet, $indir,
	$m, $M, $x, $p, $s, $r, $f);
...
#die("--infile_1 arg required\n") unless($infile_1);
#die("--infile_2 arg required\n") unless($infile_2);
...
if($indir && $sampleSheet){
	...
	for my $sampleName (keys %{$rHoAoH_sampleInfo}) {
		...
		for my $rH_laneInfo (@$rAoH_sampleLanes) { #this rH_laneInfo contains the complete line info from the sample sheet for this sample.
			...
			#$R1_gz .= $rawReadDir .'/' .$sampleName .'/run' .$rH_laneInfo->{'runId'} . "_" . $rH_laneInfo->{'lane'} .'/' . $sampleName . "." . $rH_laneInfo->{'libraryBarcode'} . $rH_laneInfo->{'read1File'};
			#$R2_gz .= $rawReadDir .'/' .$sampleName .'/run' .$rH_laneInfo->{'runId'} . "_" . $rH_laneInfo->{'lane'} .'/' . $sampleName . "." . $rH_laneInfo->{'libraryBarcode'} . $rH_laneInfo->{'read2File'};
...
my ($qscores1, $qscores2, $qscoresAss) = generateQscoreSheet(\%hash1, \%hash2, \%hashAssembled);	
generateQscorePlots($qscores1, $qscores2, $qscoresAss);
...
sub getStatsForAssembly{
	...
	# Reads 1
    my $counter = 0;
    my $in = new Iterator::FastqDb($reads_1, {trim3=>0, trimN=>0}) or die("Unable to open Fastq file, $reads_1\n");
	...
	# Reads 2
    $counter = 0;
    $in = new Iterator::FastqDb($reads_2, {trim3=>0, trimN=>0}) or die("Unable to open Fastq file, $reads_2\n");
    while( my $curr = $in->next_seq() and ($counter < $sampling_number) ){
        push(@readLength, length($curr->seq()));
        $counter++;
    }
	...
	$cmd .= " -r ".$averageReadLength;
	$cmd .= " -o assembled";
	$cmd .= " -d assembly_sampling";
	$cmd .= " -t ".$num_threads;
	system($cmd);
	$? != 0 ? die "command failed: $!\n" : print STDERR "FLASH assembly for sampling done...\n" if($verbose);

    $counter = 0;
    $in = new Iterator::FastqDb($outdir."/assembly_sampling/assembled.extendedFrags.fastq", {trim3=>0, trimN=>0}) or die("Unable to open Fastq file, ".$outdir."/assembly_sampling/assembled.extendedFrags.fastq");
    while( my $curr = $in->next_seq() and ($counter < $sampling_number) ){
