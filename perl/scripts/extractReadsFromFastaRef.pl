...
my $usage=<<'ENDHERE';
NAME:
extractReadsFromFastaRef.pl

PURPOSE:

INPUT:
--infiles <string>    : bam files
--fasta_ref <string>  : fasta ref
...
## OPTIONS
my ($help, $infiles, $outdir, $fasta_ref, $num_threads, $flag);
my $verbose = 0;
...
my %hash_fh1;
my %hash_unpaired;
my $ref_fasta_db = Iterator::FastaDb->new($fasta_ref) or die("Unable to open Fasta file, $fasta_ref\n");
while( my $curr = $ref_fasta_db->next_seq() ) {
   my ($contig_id) = $curr->header =~ /^>(\S+)/;
   $hash{$contig_id} = "curr_bin";
}
...
foreach(@array){
   ...
   $name =~ s{.*/}{};      # removes path  
   $name =~ s{\.[^.]+$}{}; # removes extension 
   my $prefix = $name."_unpaired_".$R_prefix;
   #my $prefix = "bin_unpaired_".$R_prefix;
