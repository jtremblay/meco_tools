...
my $usage=<<'ENDHERE';
NAME:
    blast_primer_on_seq.pl
PURPOSE:
	To blast a primer sequence (short seq) against a reference sequence database. Generated output is a table of blast data that is used as input in a following script.
INPUT:
...
my $primer_seq = new JGI::Bio::Iterator::FastaDb($primer_pair) or die("Unable to open Fasta file, $primer_pair \n");
my $fwd_primer = "";
my $rev_primer = "";
$header_counter = 0;
my $name_fwd;
...
my $ref_fasta_db = new JGI::Bio::Iterator::FastaDb($ref_db) or die("Unable to open Fasta file, $ref_db\n");
