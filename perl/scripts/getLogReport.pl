...
sub getLogTextReport {
  ...
  for my $jobLog (@AoH_jobLogList) {
    $logTextReport .= join("\t", (
      ...
      exists $jobLog->{'walltime'} ? timeToHours($jobLog-> {'walltime'}) . " (" . formatDuration($jobLog->{'duration'}) . ")" : "N/A", #REALTIME
      ...
      exists $jobLog->{'cput'} ? timeToHours($jobLog->{'cput'}) . " (" . formatDuration(timeToSeconds($jobLog->{'cput'})) . ")" : "N/A", # CPUTIME
...
sub timeToHours {
  ...
  if ($time =~ /^(\d+):(\d\d):(\d\d)/) {
    return sprintf("%.3f", ($1 * 1) + ($2 / 60) + ($3/3600) );
  } else {
    return "N/A";
  }
}
