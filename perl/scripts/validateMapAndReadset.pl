...
INPUT:
--infile_readset <string>      : Sequence file
...
## OPTIONS
my ($help, $infile_readset, $infile_mapping);
my $verbose = 0;

GetOptions(
   'infile_readset=s' 	   => \$infile_readset,
...
open(RE, "<".$infile_readset) or die "Can't open $infile_readset\n";
while(<RE>){
   next if($. == 1);
   my @row = split(/\t/, $_);
   my $header = $row[0];
   ...
   if(exists $hash_barcodes{$header}){
      print STDERR "Sample ".$header." is present multiple times in your readset file...\n";
      $redFlag = 1;
   }else{
      $hash_barcodes{$header} = "ACGT";
   }
   ...
   if(exists $hash{$header}){
      ...
      print STDERR "In readset file, sample ".$header." is not present in corresponding mapping_file...\n";
      $redFlag = 1;
   }
...
for my $key (keys %hash){
   if(exists $hash_barcodes{$key}){
      ...
      print STDERR "Entry ".$key." exists in mapping file, but not in readset file...\n";
      $redFlag = 1;
   }
}
