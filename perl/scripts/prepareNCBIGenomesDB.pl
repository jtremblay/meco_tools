...
## OPTIONS
my ($help, $infile, $outdir, $concatenate, $outdir_gff, $gff);
my $verbose = 0;
...
while(<IN>){
    ...
    my @row = split(/\t/, $_);
        ...
        if($gff){
            die "Please specify --outdir_gff when using -gff flag.\n" unless($outdir_gff);
            my $gff_url = $row[19]."/".$basename."_genomic.gff.gz";

            # Get the file
            system("wget -P $outdir_gff $gff_url");
            die "command failed: $!\n" if($? != 0);
            #if($? != 0){
            #    my $gff_url2 = $row[19]."/".$basename."_genomic.gbff.gz";
            #    system("wget -P $outdir_gff $gff_url2");
            #}
        }
