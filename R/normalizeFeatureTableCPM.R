...
performNormalizationFeatures <- function(infile, outfile, abundance_cpm) {
   library(edgeR)
   library(tools)
   library(data.table)
   ...
   tData = fread(infile, skip="FEATURE_ID", header=TRUE) # Means will start reading when find a row matching 'Feature ID'
...
usage=function(errM) {
        cat("\nUsage : Rscript normalizeFeatureTableCPM.R [option] <Value>\n")
        cat("       -i        : infile\n")
        cat("       -o        : outfile\n")
        cat("       -c        : abundance_cpm\n")
}
...
performNormalizationFeatures(infile, outfile, abundance_cpm)
