...
# Generate Feature table from shotgun metagenomics
# reads abundance data.
# National Research Council - Biomonitoring
# Author: Julien Tremblay - jtremblay514@gmail.com
generateFeatureTableRrna <- function(gene_abun_filename, taxonomy_filename, outfile) {
   taxonomy = read.table(taxonomy_filename, sep="\t", header=T, comment.char="")
   colnames(taxonomy)[1] = "gene_id"
   taxonomy$gene_id = gsub("^\\S+S_rRNA\\:\\:", "", taxonomy$gene_id)
   taxonomy$gene_id = gsub("\\(\\+\\)", "", taxonomy$gene_id)
   taxonomy$gene_id = gsub("\\(\\-\\)", "", taxonomy$gene_id)
...
usage=function(errM) {
  cat("\nUsage : Rscript generateFeatureTable.R .R [option] <Value>\n")
...
generateFeatureTableRrna(gene_abun, taxonomy, outfile)
