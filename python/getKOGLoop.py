...
def main(arguments):
    ...
    parser.add_argument('-b', '--infile-kog', help="Input file", type=argparse.FileType('r'))
    ...
    infile_kog = os.path.abspath(args.infile_kog.name)
    ...
    #q = Queue(maxsize=0)
    #get_all_files(q, matches, indir, infile_kog)
    #num_threads = 11
    p = multiprocessing.Pool(num_threads)
    for match in matches:
        p.apply_async(do_work, [match, indir, infile_kog])
    
    p.close()
    p.join()

    
def do_work(match, indir, infile_kog):    
    #param = q.get()
    #match = param[0]
    #indir = param[1]
    #infile_kog = param[2]
    ...
    #while True:
        # Do work....
    outfile = os.path.splitext(match)[0] + "_KOG.tsv"
    OUT = open(outfile, 'w')
    ...
    my_kog_file = open(infile_kog, "rU")
    #reader=csv.DictReader(fhand, delimiter="\t")
    k=0
    for line in my_kog_file:
        line = line.rstrip('\n')
        row = line.split('\t')
        gene_id     = row[0]
        cdd_id      = row[1]
        ...
        descs       = desc.split(',')
        kog_id      = descs.pop(0)
        kog_gene_id = descs.pop(0)
        kog_gene_id = re.sub('\s+', '', kog_gene_id)
   
        #sys.stderr.write("descs" + ','.join(descs) + "\n")
        #sys.stderr.write("gene_id: " + gene_id + "\n")
        #sys.stderr.write("cdd_id: " + cdd_id + "\n")
        #sys.stderr.write("kog_id: " + kog_id + "\n")
        #sys.stderr.write("kog_gene_id: " + kog_gene_id + "\n")

        hash_ko[kog_id + "==" + kog_gene_id + "==" + '.'.join(descs)].append(gene_id)
