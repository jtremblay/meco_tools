...
def main(arguments):
    ...
    for row in reader:
        ...
        #hash_cog[ko_id + "--" + definition].append(gene_id)
        hash_cog[cog].append(gene_id)
        hash_genes[gene_id] = ""
        #sys.stderr.write(ko_id + "\t" + definition + "\t" + gene_id + "\n")

    #pprint.pprint(hash_cog)
    #exit(0)

    sys.stderr.write("Completed hashing parsed COG table\n")
    ...
    #once blastp file has been parsed, loop through its hash_cog.
    #for k in iter(d.keys())
    #for ko in hash_cog.iterkeys():
    for cog in iter(hash_cog.keys()):
        # get gene id
        gene_ids = map(str, hash_cog[cog])
        #pprint.pprint(hash_cog[cog])
