...
"""Takes input in rps-blast format against TIGRFAM database. (parsed output that is...)
and an abundance (gene count) table and output a TIGRFAM abundance table.
Julien Tremblay - 
"""
...
def do_work(match, indir, infile_pfam):    
    ...
    #while True:
        # Do work....
    outfile = os.path.splitext(match)[0] + "_TIGRFAM.tsv"
    OUT = open(outfile, 'w')
    ...
    for line in my_pfam_file:
        ...
        if(line[:1] == '#'):
            ...
            if(gene_id in seen):
                ...
                hash_ko[pfam_id + "==" + desc].append(gene_id)
